...
from ids_validator.checks.generic.datacubes import DatacubesChecker, Message
...
files_to_failures = {
    ...
    "measures_values_invalid_items_type.json": [
        (Message.MEASURES_VALUES_NESTED_ARRAY_TYPE_ERROR.value, Log.CRITICAL.value)
    ],
    "measures_values_invalid_data_type.json": [
        (Message.MEASURES_VALUES_TYPE_ERROR.value, Log.CRITICAL.value)
    ],
    "dimensions_scale_invalid_type.json": [
        (Message.DIMENSIONS_SCALE_TYPE_ERROR.value, Log.CRITICAL.value)
    ],
...
def test_datacubes_check(fname, expected, mocker):
    ...
    node = Node(name="datacubes", ids_dict=datacubes, path="root.properties.datacubes")
